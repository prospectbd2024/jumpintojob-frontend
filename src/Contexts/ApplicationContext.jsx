...
function ApplicationContext({ children }) {
  ...
  const getResume = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage({
          type: "error",
          message: `Failed to fetch resume: ${errorData.message || response.statusText}`,
        });
        return;
      }
  ...
  const apply = async (job, cv, resume) => {
    ...
    try {
      ...
      formData.append("forwarding_letter_type", forwardingLetter.type ?? "None");
      ...
      {
        // "forwarding_letter_type" : "no cover later",
        //    "applicant_status" : "available",
        //    "forwarding_letter" : "done"
      }
      const bearerToken = userData.data.access_token;
